 ...
 *******************************************************************************/
 ...
 // Set location
 // 25.099150626827242, 55.16040026752094
 float lat=25.099150626827242;
 float lon=55.16040026752094;
 int   alt=0;
...
// Receive UDP packages and process
...
int UDP_SendGWStatusUpate()
{
  ...
  if (nowseconds - Status_lasttime >= 30) {
...
int UDP_SendPullData()
{
  struct timeval nowtime;

  gettimeofday(&nowtime, NULL);
  uint32_t nowseconds = (uint32_t)(nowtime.tv_sec);
  if (nowseconds - PushData_lasttime >= 5) {
